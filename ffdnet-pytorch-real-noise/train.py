...
def main(args):
	...
	for epoch in range(start_epoch, args.epochs):
		...
		for i, data in enumerate(loader_train, 0):
			...
			img_train = Variable(img_train.cpu())
			imgn_train = Variable(imgn_train.cpu())
			noise = Variable(noise.cpu())
			stdn_var = Variable(torch.FloatTensor(stdn))
		...
		for valimg in dataset_val:
			...
			img_val, imgn_val = Variable(img_val.cpu()), Variable(imgn_val.cpu())
			sigma_noise = Variable(torch.FloatTensor([args.val_noiseL]))
		...
		print('PSNR on validation data', psnr_val, epoch)
		print('Learning rate', current_lr, epoch)
