...
def test_ffdnet(**args):
	...
	if rgb_den:
		...
		noise_map = cv2.imread(args['noise_map'], cv2.IMREAD_GRAYSCALE)
		noise_map = np.float32(noise_map)
		noise_map /= 255.
		noise_map = np.expand_dims(noise_map, 0)
	imorig = np.expand_dims(imorig, 0)
	noise_map = np.expand_dims(noise_map, 0)
	# Handle odd sizes
	expanded_h = False
	...
	if sh_im[2]%2 == 1:
		...
		noise_map = np.concatenate((noise_map, \
				noise_map[:, :, -1, :][:, :, np.newaxis, :]), axis=2)
	...
	with torch.no_grad(): # PyTorch v0.4.0
	    ...
	    noise_map = Variable(noise_map.type(dtype))

	# Measure runtime
	start_t = time.time()

	# s = 255 * nle.noiseLevel(imnoisy, method='MAD')
	# print(s)
	...
	if not args['dont_save_results']:
		...
		cv2.imwrite('savedImage/ori-'+args['input'], noisyimg)
		cv2.imwrite('savedImage/deno-'+args['input'], outimg)
		if args['add_noise']:
