...
def concatenate_input_noise_map(input, noise_sigma):
	...
	if 'cuda' in dtype:
		...
		noise_map = torch.FloatTensor(1, C, Hout, Wout).fill_(0)

	# Build the CxH/2xW/2 noise map
	# noise_sigma.view(N, 1, 1, 1).repeat(1, C, Hout, Wout)
	for i in range(Hout):
		for j in range(Wout):
			noise_map[0, 0, i, j] = (noise_sigma[:,:,2*i,2*j]+noise_sigma[:,:,2*i+1,2*j]
				+noise_sigma[:,:,2*i,2*j+1]+noise_sigma[:,:,2*i+1,2*j])/4
	print(noise_map)
